...
public class WebSocketImpl implements WebSocket {
		...
		try {
			...
			for( Framedata f : frames ) {
				if( DEBUG )
					...
					Logger.getLogger(WebSocketImpl.class).debug("matched frame: " + f );
	...
	public void sendFrame( Framedata framedata ) {
		if( DEBUG )
			...
			Logger.getLogger(WebSocketImpl.class).debug( "send frame: " + framedata);
	...
	private void write( ByteBuffer buf ) {
		...
		//if( DEBUG )
			...
			Logger.getLogger(WebSocketImpl.class).debug("write(" + buf.remaining() + "): {" + ( buf.remaining() > 1000 ? "too big to display" : new String(buf.array())).substring(2) + "}");
		outQueue.add( buf );
		/*try {
			outQueue.put( buf );
	...
	private void open( Handshakedata d ) {
		if( DEBUG )
			...
			Logger.getLogger(WebSocketImpl.class).debug( "open using draft: " + draft.getClass().getSimpleName());
